...
<script type="text/javascript">
	...
	$(document).ready(function() {
		$(".card").each(
			function() {
				const activityIdValue = $(this).find("#activityIdValue").val();
				const categoryValue = $(this).find("#categoryValue").val();

				if (categoryValue == "여행 - 모험") {
					$(this).css("border", "5px solid #64CCC5");
				} else if (categoryValue == "문화 - 엔터테인먼트") {
					$(this).css("border", "5px solid #F5E8B7");
				}
		...
		<table>
			...
			<tbody>
				...
				<c:forEach var="bean" items="${requestScope.personalActivity}" varStatus="loopStatus">
					...
					<td>
						<div class="card card${bean.activityId}">
							<input type="hidden" id="activityIdValue" value="${bean.activityId}">
							<input type="hidden" id="categoryValue" value="${bean.category}">
							...
							<div class="card-body">
								<a href="<%=notWithFormTag%>activityDetail&activityId=${bean.activityId}" class="activityImage-a-tag">
